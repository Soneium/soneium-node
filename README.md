...
1. **Set Environment Variables**
   ...
   Create  `${NETWORK}/env` file based on `env.default` in a text editor and set the following variables:
...
4. **Run Docker Compose**
   ...
   ```bash
   NETWORK=hari docker compose up -d
   ```
...
5. **Check Logs**
   ...
   - For `op-node-minato`:
     ```bash
     NETWORK=hari docker compose logs -f op-node
     ```
  ...
  - For `op-geth`:
     ```bash
     NETWORK=hari docker-compose logs -f op-geth
     ```
